...
public sealed class CMUAutodocBui : BoundUserInterface
{
    ...
    {
        if (_selectedPart is { } selected)
        {
            foreach (var candidate in state.Parts)
            {
                if (!PartMatches(candidate, selected))
                    continue;
    ...
    private static bool ContainsPart(CMUAutodocBuiState state, (NetEntity Part, BodyPartType Type, BodyPartSymmetry Symmetry) key)
    {
        foreach (var part in state.Parts)
        {
    ...
    {
        return part.Part == key.Part && part.Type == key.Type && part.Symmetry == key.Symmetry;
    }
